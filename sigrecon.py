...
def objective_func(x0, path, x_fixed, m_i, n_fft):
  ...
  return np.abs(fft.fft(x, n_fft)[:int(n_fft / 2 + 1)]) - m_i
...
def stft(y, n_fft=512, hop_length=64, win_length=256, dtype=np.complex64, window_type='boxcar'):
  ...
  stft_m = np.empty((int(n_fft / 2 + 1), int((y.shape[0] - win_length) / hop_length + 1)), dtype=dtype)
  ...
  for i in range(stft_m.shape[1]):
    ...
    # Calculate FFT
    stft_m[:, i] = fft.fft(y_frame, n_fft)[:n_fft // 2 + 1]
